...
window.addEventListener('DOMContentLoaded', async() => {   //on loading screen
      let x;
      try{
        x=await axios.get(`${url}/get-candy`);
        console.log(x);
      }
      catch{
       console.log('ERROR');
...
async function onFormSubmit(e) {
	event.preventDefault();
        ...
        if (selectedRow == null){
            ...
            try{
                t=await axios.post(`${url}/post-candy`,formData)
...
async function onDelete(td) {
    // if (confirm('Do you want to delete this record?')) {
         ...
         try{
             ...
             t=await axios.get(`${url}/delete-candy/${td}`)
             console.log(t);
             //remove(td);
         }
         catch{
...
async function post(newCandyQty){
    let t;
    try{
        t = await axios.post(`${url}/post-candy`,newCandyQty);
    }
